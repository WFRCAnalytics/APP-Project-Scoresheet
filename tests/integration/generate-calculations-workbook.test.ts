...
// Overall Wtd/City Wtd values landed one column left of their own header) — this test
// specifically asserts the totals row lines up under the SAME header text it's meant to be
// under, not just that some value exists somewhere in the row.
...
describe("generateCalculationsWorkbook", () => {
  it("has a header row matching Firm/Criterion/Weight/[reviewers]/Overall Avg/City Avg/Overall Wtd/City Wtd/Completion", async () => {
  ...
  it("puts each firm's totals row values directly under the Overall Wtd / City Wtd header columns, not offset", async () => {
    const project = buildFixture();
    const { blob } = await generateCalculationsWorkbook(project);
    const workbook = await reloadWorkbook(blob);
    const sheet = workbook.getWorksheet("Calculations")!;

    // Locate the header row's Overall Wtd / City Wtd columns dynamically, rather than
    // hardcoding a column letter a second time in this test.
    ...
    for (let c = 1; c <= headerRow.cellCount; c++) {
      ...
      if (value === "City Wtd") cityWtdCol = c;
    }
    expect(overallWtdCol).toBeGreaterThan(0);
    expect(cityWtdCol).toBeGreaterThan(0);
    ...
    // Overall Wtd = 3*0.6 + 5*0.4 = 3.8; City Wtd (Alice only) = 5*0.6 + 5*0.4 = 5.
    expect(totalsRow.getCell(overallWtdCol).value).toBe(3.8);
    expect(totalsRow.getCell(cityWtdCol).value).toBe(5);
    // And the cell immediately left of Overall Wtd (City Avg) must NOT hold the totals
